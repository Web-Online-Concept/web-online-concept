...
export default function ShareButtons({ url, title, description }) {
  ...
  const handleShare = (e, shareUrl, platform) => {
    e.preventDefault()
    
    // Pour WhatsApp sur mobile, ouvrir dans l'app
    if (platform === 'WhatsApp' && /Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
      window.location.href = shareUrl
    } else if (platform === 'Email') {
      window.location.href = shareUrl
    } else {
      // Pour les autres, ouvrir dans une popup
      window.open(
        shareUrl,
    ...
    <div className="flex items-center justify-center gap-2">
      ...
      {shareLinks.map((link) => (
        <a
          key={link.name}
          href={link.url}
          onClick={(e) => handleShare(e, link.url, link.name)}
          className={`p-2 rounded-full text-gray-600 border border-gray-300 transition-all duration-200 ${link.color}`}
